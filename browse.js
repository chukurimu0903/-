...
for(j=0;j<target_url_list.length;j++){
	...
	target_url = page_txt.match(/href=\"http\:\/\/www\.anitube\.se\/video\/[^\"]*\">/g);
	for(i=0;i<target_url.length;i++){
		target_url[i] = target_url[i].replace(/(href=|\"|>)/g, "");
	}
	target_url.sort(compareNo);
	for(i=0;i<target_url.length;i++){
		objStream.Open;
...
function compareNo(a, b){
	...
	b = b.match( /\/[0-9]+\/(.*)/ )[1];
	WScript.Echo(a);
	WScript.Echo(a.match( /[0-9]+/ ));
	a = parseInt(a.match( /[0-9]+/ ));
	b = parseInt(b.match( /[0-9]+/ ));
	return a - b;
}
